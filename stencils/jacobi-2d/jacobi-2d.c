...
void init_array (int n,
		 ...
		 DATA_TYPE POLYBENCH_2D(B_mkl,N,N,n,n))
{
  int i, j;
...
/* MKL optimized implementation of Jacobi-2D kernel */
static
void kernel_jacobi_2d_mkl(int tsteps,
                        int n,
                        DATA_TYPE POLYBENCH_2D(A,N,N,n,n),
                        DATA_TYPE POLYBENCH_2D(B,N,N,n,n))
{
  ...
  DATA_TYPE coeff = SCALAR_VAL(0.2);
  
  /* Allocate MKL memory-aligned temporary array for row operations */
  DATA_TYPE* temp_row = (DATA_TYPE*)mkl_malloc(n * sizeof(DATA_TYPE), 64);
  
  for (t = 0; t < tsteps; t++) {
    /* First sweep: B = stencil(A) */
    for (i = 1; i < n - 1; i++) {
      /* Vector operations for each row */
      #ifdef DATA_TYPE_IS_DOUBLE
      for (j = 1; j < n - 1; j++) {
        B[i][j] = coeff * (A[i][j] + A[i][j-1] + A[i][j+1] + A[i+1][j] + A[i-1][j]);
      }
    ...
    for (i = 1; i < n - 1; i++) {
      ...
      #ifdef DATA_TYPE_IS_DOUBLE
      for (j = 1; j < n - 1; j++) {
        A[i][j] = coeff * (B[i][j] + B[i][j-1] + B[i][j+1] + B[i+1][j] + B[i-1][j]);
      }
      #elif defined(DATA_TYPE_IS_FLOAT)
      for (j = 1; j < n - 1; j++) {
        A[i][j] = coeff * (B[i][j] + B[i][j-1] + B[i][j+1] + B[i+1][j] + B[i-1][j]);
...
int main(int argc, char** argv)
{
  ...
  POLYBENCH_2D_ARRAY_DECL(B_mkl, DATA_TYPE, N, N, n, n);
  ...
  double naive_time = time_kernel(kernel_jacobi_2d, tsteps, n,
                                POLYBENCH_ARRAY(A),
                                POLYBENCH_ARRAY(B));
  
  printf("Naive Jacobi-2D Time: %0.6f seconds\n", naive_time);

  /* Time MKL implementation */
  double mkl_time = time_kernel(kernel_jacobi_2d_mkl, tsteps, n,
                              POLYBENCH_ARRAY(A_mkl),
                              POLYBENCH_ARRAY(B_mkl));
  
  printf("MKL Jacobi-2D Time: %0.6f seconds\n", mkl_time);
  printf("Speedup: %0.2f\n", naive_time / mkl_time);
