...
/* Intel MKL optimized LU decomposition */
static
void kernel_lu_mkl(int n,
                 DATA_TYPE POLYBENCH_2D(A,N,N,n,n))
{
#ifdef DATA_TYPE_IS_DOUBLE
  /* Use MKL's LAPACK dgetrf function for LU decomposition */
  int info;
  int* ipiv = (int*)malloc(n * sizeof(int));
  
  /* LU decomposition with partial pivoting
     Note: LAPACK's dgetrf produces both L and U in the same matrix A */
  dgetrf(&n, &n, &A[0][0], &n, ipiv, &info);
  
  /* Handle singular matrices */
  if (info != 0) {
    printf("Warning: dgetrf returned info = %d (matrix may be singular)\n", info);
  ...
  /* Use MKL's LAPACK sgetrf function for LU decomposition with single precision */
  int info;
  int* ipiv = (int*)malloc(n * sizeof(int));
  ...
  if (info != 0) {
    printf("Warning: sgetrf returned info = %d (matrix may be singular)\n", info);
    /* Fall back to naive implementation */
    kernel_lu(n, A);
  }
...
int main(int argc, char** argv)
{
  ...
  double naive_time = time_kernel(kernel_lu, n, POLYBENCH_ARRAY(A));
  
  printf("Naive LU Time: %0.6f seconds\n", naive_time);

  /* Time MKL implementation */
  double mkl_time = time_kernel(kernel_lu_mkl, n, POLYBENCH_ARRAY(A_mkl));
  
  printf("MKL LU Time: %0.6f seconds\n", mkl_time);
  printf("Speedup: %0.2f\n", naive_time / mkl_time);
